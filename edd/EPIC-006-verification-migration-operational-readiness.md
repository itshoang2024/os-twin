...
Roles: @engineer, @qa-automation-engineer, @qa, @sre-lead, @principal-engineer
working_dir: ./
...
- [ ] Tests prove every emitted event has `plan_id` and no required separate run identifier field.
- [ ] Tests prove every room-scoped event has `room_id` and `epic_ref`.
- [ ] Tests prove channel messages carry plan identity after the migration.
...
| S19 | User cancel request | `bot`, `dashboard`, `.agents` | `user.plan.cancel_requested` appended with plan identity | integration |
| S20 | Resume failed plan | `.agents`, `dashboard`, `bot` | fresh archived execution log or explicit refusal according to EPIC-003 rule | integration |
...
- Resume preserves plan binding and follows the failed-execution reset/refusal rule.
...
- Failure notification includes plan ID, epic ref, room ID, summary, and last message preview.
...
| `channel.jsonl` | Remains conversation context; gains plan identity fields |
