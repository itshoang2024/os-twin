#!/usr/bin/env bash
# dashboard-daemon.sh — systemd wrapper for the Ostwin dashboard (Linux)
#
# This script is called by the ostwin-dashboard systemd user service.
# It sources .env for API keys, then starts the dashboard via dashboard.sh.
# Restart=on-failure in the unit file means systemd will restart this if it exits.
set -euo pipefail
...
echo "[$(date +%H:%M:%S)] Starting Ostwin dashboard (systemd) on :$DASHBOARD_PORT..."
