...
function Setup-Env {
    ...
    if (Test-Path $envFile) {
        ...
        # Backfill auto-generated keys for installs whose .env predates them
        # (or was hand-assembled). Without OSTWIN_VAULT_KEY the encrypted vault
        # cannot start; without OSTWIN_API_KEY the dashboard rejects every MCP
        # request (503 MCP_AUTH_NOT_CONFIGURED).
        Ensure-VaultKey -EnvFile $envFile
        Ensure-ApiKey -EnvFile $envFile
        Create-EnvPs1Hook
        return
    }
    ...
    # Generate the encryption key for the settings vault (mirrors setup-env.sh).
    # Without this, the encrypted_file vault backend cannot init Fernet and every
    # vault read/write (provider API keys, etc.) raises at runtime.
    $generatedVaultKey = New-VaultKey
...
# Keep this stable — changing it makes all stored secrets undecryptable.
OSTWIN_VAULT_KEY=$generatedVaultKey
...
function New-VaultKey {
    # 32 random bytes, base64-encoded — used as the passphrase the vault
    # backend feeds into PBKDF2 to derive the Fernet key. Equivalent to the
    # bash installer's `openssl rand -base64 32`.
    [CmdletBinding()]
    param()
...
function Ensure-VaultKey {
    # Append OSTWIN_VAULT_KEY to an existing .env that predates vault-key
    # auto-generation. Idempotent: does nothing when the key is already set.
    [CmdletBinding()]
    ...
    if ($lines | Where-Object { $_ -match '^\s*OSTWIN_VAULT_KEY=' }) {
        return
    }

    $vaultKey = New-VaultKey
    $append = @(
        "",
        "# Encryption key for the settings vault. Auto-generated by Ostwin.",
        "# Keep this stable — changing it makes all stored secrets undecryptable.",
        "OSTWIN_VAULT_KEY=$vaultKey"
    )
    Add-Content -Path $EnvFile -Value $append -Encoding UTF8
    Write-Ok "Added OSTWIN_VAULT_KEY to $EnvFile"
}

function New-ApiKey {
    # Dashboard <-> agent/MCP auth token: "ostwin_" + 32 url-safe base64 chars.
    [CmdletBinding()]
    param()

    $bytes = New-Object byte[] 32
    [System.Security.Cryptography.RandomNumberGenerator]::Create().GetBytes($bytes)
    $b64 = [Convert]::ToBase64String($bytes).Replace("/", "").Replace("+", "").Replace("=", "")
    return "ostwin_" + $b64.Substring(0, 32)
}

function Ensure-ApiKey {
    # Append OSTWIN_API_KEY to an existing .env that lacks it. Without it the
    # dashboard rejects every MCP request (503 MCP_AUTH_NOT_CONFIGURED) because
    # agent and dashboard both resolve the same {env:OSTWIN_API_KEY}.
    # Idempotent: does nothing when the key is already set.
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$EnvFile)
    ...
    $lines = Get-Content -Path $EnvFile
    if ($lines | Where-Object { $_ -match '^\s*OSTWIN_API_KEY=' }) {
        return
    }
    ...
    $append = @(
        "",
        "# API key for CLI/agent/MCP <-> Dashboard auth. Auto-generated by Ostwin.",
        "OSTWIN_API_KEY=$apiKey"
    )
    Add-Content -Path $EnvFile -Value $append -Encoding UTF8
    Write-Ok "Added OSTWIN_API_KEY to $EnvFile"
}
