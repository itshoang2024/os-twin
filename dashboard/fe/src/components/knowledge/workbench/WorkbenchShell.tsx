...
export default function WorkbenchShell({ model, children, rightRail, bottomRail, toolbar = true, passthrough = false }: { model: WorkbenchModel; children: React.ReactNode; rightRail?: React.ReactNode; bottomRail?: React.ReactNode; toolbar?: boolean; passthrough?: boolean }) {
  ...
  const [layoutMode, setLayoutMode] = React.useState<LayoutMode>('auto');
  const [colorBy, setColorBy] = React.useState<ColorBy>('object_type');
  const [propertyName, setPropertyName] = React.useState('');
  if (passthrough) return <section data-testid="workbench-shell" data-workbench-model={model.id} style={{ display: 'contents' }}>{children}</section>;
  return (
    <section className="workbench-shell" data-testid="workbench-shell" data-workbench-model={model.id}>
      ...
      `}</style>
      {toolbar ? <WorkbenchToolbar renderMode={renderMode} layoutMode={layoutMode} onRenderModeChange={setRenderMode} onLayoutModeChange={setLayoutMode} /> : null}
      <div className="wb-shell-grid">
        <WorkbenchDock tabs={tabs} activeTab={activeTab} onTabChange={setActiveTab}>
          ...
          {!isSpecLens && activeTab === 'histogram' ? <HistogramPanel facets={model.facets ?? []} /> : null}
          {!isSpecLens && activeTab === 'search' ? <div data-testid="workbench-search-panel">Search Around is provided by the hosted lens.</div> : null}
        ...
        <main className="wb-canvas" data-render-mode={renderMode} data-layout-mode={layoutMode}>{children}</main>
